...
function build_table(data) {
    //清空
    $("#admin-get-score-table tbody").empty();
...
//显示分数信息的总函数
function adminGetScore(page, classId, stage){
	to_page(page, classId, stage);
};

//点击查询按钮获取相应的分数信息
$("#submit-which-score").click(function(){
	adminGetScore(1, $("#which-class").val(), $("#which-stage").val());
});
...
function getUsers(classId){
	    ...
	    success: function(data){
	        ...
	        $.each(data.data,function(index, item){
                ...
                '</td>'+
                '<td>'+
                	'<input type="number" min="0" max="100" class="form-control td-score" placeholder="学生成绩分数" required="">'+
                '</td>'+
                '</tr>'
	        	)
